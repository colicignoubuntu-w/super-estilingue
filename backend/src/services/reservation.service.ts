...
export async function createReservation(
  ...
  const operatingDay =
    await prisma.operatingDay.findUnique({
      where: {
  ...
  // Conta somente reservas PAGAS
  const totalReservado =
    operatingDay.reservations
      .filter(
        reservation =>
  ...
  if (
    totalReservado + data.quantity >
    operatingDay.capacity
  ) {
    throw new Error(
      "Não existem vagas suficientes"
    );
  }
